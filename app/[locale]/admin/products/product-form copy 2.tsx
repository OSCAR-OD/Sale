    ...
    try {
      const uploadedImages = await Promise.all(
        acceptedFiles.map(async (file) => {
          ...
          formData.append('folder', 'sale')
          const res = await fetch('https://api.cloudinary.com/v1_1/debunniyq/image/upload', {
            method: 'POST',
            body: formData,
          })
          ...
          <FormField
            ...
            render={() => (
              ...
              <CardContent className='space-y-2 mt-2 min-h-48'>
                <div className='flex flex-wrap gap-2'>
                  {images.map((image: string, index: number) => (
                    <div key={index} className='relative w-20 h-20'>
                      <Image src={image} alt='product image' width={100} height={100} className='w-20 h-20 object-cover rounded-sm' />
                ...
                <div {...getRootProps()} className={`border-2 border-dashed p-4 cursor-pointer ${uploading ? 'opacity-50' : ''}`}>
                  <input {...getInputProps()} />
